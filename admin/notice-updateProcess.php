...
Database::iud("UPDATE `notice` SET `url` = '".$url."' WHERE `id` = '".$id."' ");
echo("success");
if (isset($_FILES["image"])) {
    $image = $_FILES["image"];

    // Retrieve current image path from the database
    $current_images_query = Database::search("SELECT `path` FROM `notice` WHERE `id`='" . $id . "'");
    ...
    if (!in_array($file_ex, $allowed_image_extensions)) {
        ...
        $file_name = "web/notice/" . uniqid() . $new_file_extension;
        ...
        if (move_uploaded_file($image["tmp_name"], $file_name)) {
            ...
            Database::iud("UPDATE `notice` SET `path` = '".$file_name."' WHERE `id` = '".$id."'");
